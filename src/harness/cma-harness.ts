...
export function createCmaHarness(opts: CmaHarnessOptions = {}): Harness {
  ...
  const selfHosted = Boolean(opts.environmentKey && opts.environmentId);
  ...
  const ensureClient = (): CmaClient => {
    ...
    if (!opts.apiKey || !opts.environmentId) {
      throw new NonRetryableTurnError(
        "The CMA harness is not configured — set CMA_ENVIRONMENT_ID and ANTHROPIC_API_KEY.",
      );
    }
    client = createCmaClient({
  ...
  const ensureAgent = async (api: CmaClient, model: string, effort: string | undefined): Promise<string> => {
    ...
    const name = ["qm", model, effort].filter(Boolean).join(" ");
  ...
  const runPrompt = async (turn: HarnessTurnInput): Promise<HarnessTurnResult> => {
    ...
    const sessionTools: CmaSessionTool[] = selfHosted ? [CMA_NATIVE_TOOLSET, ...customTools] : customTools;
    const provision = () =>
      ...
      if (stopReason?.type === "end_turn" || stopReason === undefined) {
        ...
        if (steerQueue.length) {
          ...
          await sendEventsWithRetry(
            ...
            (unseen) =>
              steers.every((steer) =>
                unseen.some((event) => event.type === "user.message" && cmaBlockText(event.content) === steer.text),
              ),
          );
    ...
    try {
      if (selfHosted) releaseWork = ensureAttendant().beginTurn(cmaSessionId);
