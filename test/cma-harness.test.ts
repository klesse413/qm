...
function selfHostedScenario(finishDelayMs: number) {
...
function selfHostedHarness(state: FakeCmaState) {
  return cmaHarness(state, { environmentKey: "sk-ant-oat01-test" });
}

test("CMA self-hosted mode runs native bash through the shared execute tool and holds the lease for the turn", async (t) => {
  const fake = await startFakeCma(selfHostedScenario(500));
  t.after(fake.close);
  const harness = selfHostedHarness(fake.state);
...
test("CMA strict posture gates native bash exactly like custom tools", async (t) => {
  const fake = await startFakeCma(selfHostedScenario(100));
  t.after(fake.close);
  const harness = selfHostedHarness(fake.state);
  t.after(async () => harness.turns.close?.());
  const executed: string[] = [];
...
test("the work attendant leaves work for sessions it is not running unclaimed", async (t) => {
  const fake = await startFakeCma(selfHostedScenario(1_800));
  t.after(fake.close);
