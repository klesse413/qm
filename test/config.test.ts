...
test("HARNESS=cma requires the CMA session plumbing and an API key", () => {
  const cmaEnv = {
    HARNESS: "cma",
  ...
  const loaded = loadConfig({ ...cmaEnv, CMA_MODEL: "claude-sonnet-5", CMA_ENVIRONMENT_KEY: "sk-ant-oat01-x" });
  ...
  assert.equal(loadConfig(cmaEnv).cmaEnvironmentKey, undefined);
  ...
  assert.throws(() => loadConfig({ HARNESS: "cma", ANTHROPIC_API_KEY: "sk-ant" }), /requires CMA_ENVIRONMENT_ID/);
  assert.doesNotThrow(
  ...
  assert.throws(
    () => loadConfig({ HARNESS: "cma", CMA_ENVIRONMENT_ID: "env_1" }),
