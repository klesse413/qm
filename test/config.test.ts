...
test("HARNESS=cma requires the CMA session plumbing and an API key", () => {
  ...
  const loaded = loadConfig({ ...cmaEnv, CMA_MODEL: "claude-sonnet-5", CMA_VAULT_IDS: "vlt_1, vlt_2" });
  ...
  assert.deepEqual(loaded.cmaVaultIds, ["vlt_1", "vlt_2"]);
  ...
  assert.throws(
    () => loadConfig({ HARNESS: "cma", ANTHROPIC_API_KEY: "sk-ant" }),
    /CMA_ENVIRONMENT_ID and CMA_AGENT_ID/,
  );
  assert.throws(
    () => loadConfig({ HARNESS: "cma", CMA_ENVIRONMENT_ID: "env_1", CMA_AGENT_ID: "agent_1" }),
